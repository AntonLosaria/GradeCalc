...
double calculateClassStanding(int grades[], int weights[], int percent) {
    double classStanding = 0.0;
    
    for (int i = 0; i < grades[i]; i++) {
        classStanding = (grades[i] / weights[i] ) * percent ; 
    }
...
int enterActivity(string name, double percent, double total, int hw[], int over[]) {
    ...
    if (val == true) 
	{
		...
		double classStanding= calculateClassStanding(hw, over, percent);
